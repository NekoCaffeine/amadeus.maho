...
public class ValueBasedHandler extends JavacContext {
    ...
    Name ObjectHelper = name(ObjectHelper.class);
    ...
    private static Hook.Result visitBinary(final Attr $this, final JCTree.JCBinary tree) {
        if (!(tree instanceof EqualsAndHashCodeHandler.IdentityCompareBinary))
            instance(ValueBasedHandler.class).transform($this, tree);
        return Hook.Result.VOID;
    }
    
    private void transform(final Attr $this, final JCTree.JCBinary tree) {
        final Symbol.ClassSymbol symbol = symtab.enterClass(mahoModule, ObjectHelper);
        try {
            symbol.complete();
            ...
            if (eq != null) {
                ...
                if (tree.lhs.type != botType && tree.rhs.type != botType && (Privilege) $this.isValueBased(tree.lhs.type)) {
                    if (tree.lhs.type.tsym == tree.rhs.type.tsym) {
                        ...
                        final JCTree.JCMethodInvocation apply = maker.Apply(List.nil(), maker.Select(maker.Ident(symbol), instance.name(eq ? "valueBasedEquals" : "valueBasedNotEquals")), List.of(tree.lhs, tree.rhs));
