...
public interface AnnotationExtension {
    ...
    @Redirect(targetClass = AnnotationInvocationHandler.class, selector = ASMHelper._INIT_, slice = @Slice(@At(insn = @At.Insn(opcode = ATHROW))), metadata = @TransformMetadata(aotLevel = AOTTransformer.Level.RUNTIME))
    private static void extendedAnnotationType(final Throwable throwable) { }
    
    @Hook(value = AnnotationParser.class, isStatic = true, at = @At(method = @At.MethodInsn(name = "isAnnotation")), before = false, capture = true, metadata = @TransformMetadata(aotLevel = AOTTransformer.Level.RUNTIME))
