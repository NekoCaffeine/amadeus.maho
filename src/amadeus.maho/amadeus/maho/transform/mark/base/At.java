...
public @interface At {
    ...
    class Lookup {
                    ...
                    while (offset != 0)
                        if (offset < 0) {
                            if (!tmpIterator.hasPrevious())
                                throw new IllegalArgumentException("can't offset to " + at.offset());
        ...
        public Predicate<AbstractInsnNode> mapPredicate(final At at) {
            final var handler = asOneOfUs(at);
            final List<? extends Annotation> annotations = handler.values()
                    .map(Map.Entry::getValue)
                    .cast(Annotation.class)
                    .toList();
            if (annotations.size() > 1)
                throw new IllegalArgumentException(at + " containing repeating elements");
            final Annotation annotation = annotations.size() == 0 ? DEFAULT_AT : annotations[0];
            if (annotation instanceof Endpoint endpoint)
                return switch (endpoint.value()) {
                    case HEAD      -> {
            ...
            if (annotation instanceof FieldInsn fieldInsn) {
                ...
                final var methodInsnHandler = asOneOfUs(fieldInsn);
                ...
                return insnNode -> insnNode instanceof FieldInsnNode it &&
            ...
            if (annotation instanceof MethodInsn methodInsn) {
                final String name, desc;
                if (!at.remap())
                ...
                final var methodInsnHandler = asOneOfUs(methodInsn);
                ...
                if (!at.remap())
                    ...
                    desc = remapper.mapDesc(sourceDesc);
                return insnNode -> insnNode instanceof MethodInsnNode it &&
            ...
            if (annotation instanceof TypeInsn typeInsn) {
                final String desc;
                final var methodInsnHandler = asOneOfUs(typeInsn);
                ...
                return insnNode -> insnNode instanceof TypeInsnNode it &&
                        (typeInsn.opcode() == WILDCARD_INT || typeInsn.opcode() == it.getOpcode()) &&
                        (desc.equals(WILDCARD) || desc.equals(it.desc));
            }
            if (annotation instanceof Insn insn)
                return insnNode -> insnNode instanceof InsnNode it && (insn.opcode() == WILDCARD_INT || insn.opcode() == it.getOpcode());
            if (annotation instanceof AnyInsn anyInsn)
                return insnNode -> anyInsn.opcode() == WILDCARD_INT || anyInsn.opcode() == insnNode.getOpcode();
            if (annotation instanceof IntInsn intInsn)
                return insnNode -> insnNode instanceof IntInsnNode it &&
                        (intInsn.opcode() == WILDCARD_INT || intInsn.opcode() == it.getOpcode()) &&
                        (intInsn.operand() == WILDCARD_INT || intInsn.operand() == it.operand);
            if (annotation instanceof InvokeDynamicInsn invokeDynamicInsn) {
                final String name, desc;
                if (!at.remap())
                ...
                final var invokeDynamicInsnHandler = asOneOfUs(invokeDynamicInsn);
                ...
                if (!at.remap())
                    ...
                    desc = remapper.mapDesc(sourceDesc);
                return insnNode -> insnNode instanceof InvokeDynamicInsnNode it &&
                        (invokeDynamicInsn.opcode() == WILDCARD_INT || invokeDynamicInsn.opcode() == it.getOpcode()) &&
                        (name.equals(WILDCARD) || name.equals(it.name)) &&
                        (desc.equals(WILDCARD) || desc.equals(it.desc));
            }
            if (annotation instanceof LdcInsn ldcInsn) {
                final var ldcInsnHandler = asOneOfUs(ldcInsn);
                ...
                if (mapping.size() > 1)
                    throw new IllegalArgumentException(ldcInsn + " containing repeating elements");
            ...
            if (annotation instanceof MultiANewArrayInsn multiANewArrayInsn) {
                final String desc;
                if (!at.remap())
                ...
                return insnNode -> insnNode instanceof MultiANewArrayInsnNode it &&
            ...
            if (annotation instanceof VarInsn varInsn)
                return insnNode -> insnNode instanceof VarInsnNode it &&
                        (varInsn.opcode() == WILDCARD_INT || varInsn.opcode() == it.getOpcode()) &&
                        (varInsn.var() == WILDCARD_INT || varInsn.var() == it.var);
            if (annotation instanceof IincInsn iincInsn)
                return insnNode -> insnNode instanceof IincInsnNode it &&
            ...
            if (annotation instanceof LineNumber lineNumber)
                return insnNode -> insnNode instanceof LineNumberNode it &&
